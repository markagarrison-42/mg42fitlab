...
function buildSessions(allLogs,workouts){
  ...
  Object.entries(allLogs).forEach(([exId,entries])=>{
    let exName=customNames[exId]||exId.replace(/_/g,' ').replace(/\b\w/g,l=>l.toUpperCase());
    Object.values(workouts).forEach(w=>{const found=w.exercises&&w.exercises.find(e=>e.id===exId);if(found)exName=found.name;});
    entries.forEach(entry=>{all.push({...entry,exId,exName});});
  });
...
const DEFAULT_WORKOUTS={
  ...
  anthropic_push_heavy:{label:'Push (Heavy)',tag:'Heavy',category:'push',gym:'anthropic',wtype:'push',note:'Smith machine heavy push day.',exercises:[
    {id:'bench_press_smith',name:'Bench Press (Smith)',sets:4,reps:'6-8'},
  ...
  anthropic_pull_heavy:{label:'Pull (Heavy)',tag:'Heavy',category:'pull',gym:'anthropic',wtype:'pull',note:'Heavy pull day.',exercises:[
    {id:'lat_pulldown_heavy',name:'Lat Pulldown (Heavy)',sets:4,reps:'6-8'},
  ...
  anthropic_legs_heavy:{label:'Legs (Heavy)',tag:'Heavy',category:'legs',gym:'anthropic',wtype:'legs',note:'Heavy leg day.',exercises:[
    {id:'hack_squat',name:'Hack Squat',sets:4,reps:'6-8'},
...
function ExerciseBlock({ex,accent,allLogs,restDefaults,onSetLogged,onPR}){
  ...
  function logSet(i){
    ...
    const entry={date:new Date().toISOString(),weight:w,reps:r,e1rm:est,type:setData[i].type||'normal'};
    const next=[...prevLogs,entry];
  ...
  return React.createElement('div',{style:{paddingBottom:80}},
    ...
    React.createElement('div',{style:{padding:'8px 16px'}},
      gyms.map(gym=>{
        ...
        return React.createElement('div',{key:gym,style:{marginBottom:12}},
          React.createElement('div',{onClick:()=>setExpandedGym(isOpen&&expandedGym===gym?null:gym),style:{display:'flex',alignItems:'center',gap:8,padding:'8px 0',cursor:'pointer',WebkitTapHighlightColor:'transparent'}},
            React.createElement('div',{style:{fontSize:13,fontWeight:700,color:T.sub,flex:1}},gymLabel),
            React.createElement('div',{style:{fontSize:11,color:T.dim}},list.length+' routines'),
            React.createElement('div',{style:{fontSize:14,color:T.dim}},isOpen&&expandedGym===gym?'⌃':'⌄')
          ),
          (isOpen||expandedGym!==gym)&&React.createElement('div',{style:{display:'flex',flexDirection:'column',gap:6}},
            list.map((w,idx)=>{
              const tc=TYPE_COLORS[w.wtype]||CAT[w.category]||'#64748b';
              return React.createElement('div',{key:w.key,draggable:reordering,onDragStart:reordering?()=>handleDragStart(gym,idx):undefined,onDragOver:reordering?(e)=>handleDragOver(e,gym,idx):undefined,onDragEnd:reordering?handleDragEnd:undefined,style:{background:T.bg2,borderRadius:10,border:'1px solid '+T.border,padding:'12px 14px',display:'flex',alignItems:'center',gap:10,cursor:reordering?'grab':'default',userSelect:reordering?'none':'auto'}},
                reordering&&React.createElement('div',{style:{fontSize:18,color:T.dim,flexShrink:0}},'☰'),
                React.createElement('div',{style:{width:3,borderRadius:2,alignSelf:'stretch',background:tc,flexShrink:0}}),
                React.createElement('div',{style:{flex:1,minWidth:0}},React.createElement('div',{style:{fontSize:14,fontWeight:600,color:T.text,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}},w.label),React.createElement('div',{style:{fontSize:11,color:T.dim,marginTop:2}},(w.exercises?w.exercises.length:0)+' exercises')),
                !reordering&&React.createElement('div',{style:{fontSize:10,color:tc,fontWeight:700,padding:'3px 7px',borderRadius:6,background:tc+'18',border:'1px solid '+tc+'30',flexShrink:0}},TYPE_LABELS[w.wtype]||w.tag||''),
                archiving
                  ?React.createElement('button',{
                    onClick:()=>setSelected(s=>{const n=new Set(s);n.has(w.key)?n.delete(w.key):n.add(w.key);return n;}),
                    style:{width:36,height:36,borderRadius:8,border:'2px solid '+(selected.has(w.key)?'#f87171':T.border2),background:selected.has(w.key)?'rgba(239,68,68,0.2)':'rgba(255,255,255,0.03)',color:selected.has(w.key)?'#f87171':T.dim,fontSize:16,cursor:'pointer',WebkitTapHighlightColor:'transparent',flexShrink:0,display:'flex',alignItems:'center',justifyContent:'center'}
                  },selected.has(w.key)?'✓':'○')
                  :reordering
                  ?React.createElement('div',{style:{fontSize:11,color:T.dim,flexShrink:0}})
                  :React.createElement('div',{style:{display:'flex',gap:6,flexShrink:0}},
                    React.createElement('button',{onClick:()=>setEditingRoutine(w.key),style:{padding:'7px 10px',borderRadius:8,border:'1px solid '+T.border2,background:'rgba(124,58,237,0.1)',color:'#a78bfa',fontSize:12,fontWeight:600,cursor:'pointer',WebkitTapHighlightColor:'transparent',minHeight:36}},'Edit'),
                    React.createElement('button',{onClick:()=>onStartWorkout(w.key),style:{padding:'7px 12px',borderRadius:8,border:'none',background:GRAD.button,color:'#fff',fontSize:12,fontWeight:700,cursor:'pointer',WebkitTapHighlightColor:'transparent',minHeight:36}},'▶')
                  )
              );
            })
          )
...
function ExerciseDatabase({workouts,restDefaults,onSaveRestDefaults,onSaveExercise}){
  const[search,setSearch]=useState('');
  ...
  Object.entries(workouts).forEach(([wKey,w])=>{
    (w.exercises||[]).forEach((ex,exIdx)=>{
      if(!seen.has(ex.id)){
        seen.add(ex.id);
        allExercises.push({...ex,workoutKey:wKey,workoutLabel:w.label,exIdx,category:w.category});
      }
    });
  });
  allExercises.sort((a,b)=>a.name.localeCompare(b.name));
  const filtered=allExercises.filter(ex=>!search||ex.name.toLowerCase().includes(search.toLowerCase()));

  function openEdit(ex){
    setEditingEx(ex);
    setEditDraft({name:ex.name,sets:ex.sets,reps:ex.reps,restSec:restDefaults[ex.id]||restDefaults._default||120});
  }
  function saveEdit(){
  ...
  return React.createElement('div',{style:{paddingBottom:80}},
    React.createElement('div',{style:{padding:'16px 16px 8px',position:'sticky',top:0,zIndex:5,background:T.bg,borderBottom:'1px solid '+T.border}},
      React.createElement('div',{style:{fontSize:20,fontWeight:700,color:T.text,marginBottom:12}},'Exercise Database'),
      React.createElement('input',{type:'text',placeholder:'Search exercises...',value:search,onChange:e=>setSearch(e.target.value),style:{width:'100%',padding:'10px 14px',background:T.bg3,border:'1px solid '+T.border2,borderRadius:10,color:T.text,fontSize:14,fontFamily:T.mono,minHeight:44}})
    ),
    React.createElement('div',{style:{padding:'8px 16px'}},
      ...
      filtered.map(ex=>{
        ...
        return React.createElement('div',{key:ex.id,style:{display:'flex',alignItems:'center',gap:12,padding:'13px 14px',marginBottom:6,background:T.bg2,borderRadius:10,border:'1px solid '+T.border}},
          ...
          React.createElement('div',{style:{flex:1,minWidth:0}},
            ...
            React.createElement('div',{style:{fontSize:11,color:T.dim,marginTop:2}},ex.sets+'x'+ex.reps+'  ·  '+restLabel+' rest  ·  '+ex.workoutLabel)
          ),
...
function ActiveExBlock({ex,allLogs,workout,restDefaults,handleSetLogged,handlePR,setSetsLogged,setVolumeLogged,restLabel,onMenu}){
  const prevLogs=allLogs[ex.id]||[];
  const bestE1rm=getBestE1rm(prevLogs);
  const lastSession=prevLogs.slice(-ex.sets);
...
function PPLTracker(){
  ...
  const[schedule,setScheduleRaw]=useState(()=>loadLS('fitlog_schedule',null)||DEFAULT_SCHEDULE);
  ...
  if(activeWorkout){
    ...
    return React.createElement('div',{style:{minHeight:'100vh',background:T.bg,color:T.text,fontFamily:T.sans,maxWidth:680,margin:'0 auto',paddingBottom:120,WebkitOverflowScrolling:'touch'}},
      ...
      React.createElement('div',null,getActiveExercises().map((ex,exIdx)=>React.createElement(ActiveExBlock,{key:wKey+'-'+ex.id+'-'+exIdx,ex,allLogs,workout,restDefaults,handleSetLogged,handlePR,setSetsLogged,setVolumeLogged,restLabel,onMenu:(ex)=>setExMenu({ex,exIdx})}))),
  ...
  return React.createElement('div',{style:{minHeight:'100vh',background:T.bg,color:T.text,fontFamily:T.sans,maxWidth:680,margin:'0 auto',paddingBottom:T.tabH}},
    ...
    tab==='workout'&&React.createElement(React.Fragment,null,
      ...
      React.createElement('div',{style:{padding:'4px 0 16px'}},
        workout.exercises.map(ex=>React.createElement(ExerciseBlock,{key:wKey+'-'+ex.id,ex,accent,allLogs,restDefaults,onSetLogged:handleSetLogged,onPR:handlePR}))
      )
    ),
